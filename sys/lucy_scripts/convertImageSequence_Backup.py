...
PROJECT_DEFAULTS = {'colorspace': 'Linear', 'x': 1080, 'y': 1080, 'pixelAspectRatio': 2.4, 'filter': 0, 'CDL': 0}
...
def importPlateAction(*args, **kwargs):
    ...
    outformat = "%s %s %s" %(workingValues['x'],  workingValues['y'],  workingValues['pixelAspectRatio'])
    ...
    if not mavisEntity.has_key("working"):
        #workingValues = {'colorspace': 'Linear', 'wX': 1920, 'wY': 1080, 'inPixelAspect': 2.4, 'inFilter': 0, 'inCDL': 0}
        workingValues = {'colorspace': 'Linear', 'x': 1080, 'y': 1080, 'pixelAspectRatio': 2.4, 'filter': 0, 'CDL': 0}
        workingValues        = PROJECT_DEFAULTS
    else:
    ...
    if copy.copy(inputValues).update(workingValues) != inputValues:
        ...
        if inputValues['x'] != workingValues['x'] or inputValues['y'] != workingValues['y']:
            ...
            dst.knob("resize").setValue( "%s %s %s"%(str(workingValues['x']),  str(workingValues['y']),  str(workingValues['pixelAspectRatio']) ))  
        ...
        if str(workingValues['colorspace']) == "Linear":
        ...
        if not connectNodes(nodeFlow[-2], proxyResize, 0): #NB - this may need to be -2 not the write node, but the one above
            ...
            proxyX=int(float(workingValues['x'])/2)
            proxyY=int(float(workingValues['y'])/2)
